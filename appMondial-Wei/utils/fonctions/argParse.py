...
def argParse(managerRes, formData):
    ...
    parser.add_argument("-TC", type=str,
                        help="ID du type de cuisine. Choix : Europe(eu), Asie (azy), Amérique du Nord/Sud (as/an), Afrique (af), VIP (vip). Veuillez entrer l'id et non le nom complet.")
    ...
    if args.r and managerRes:
        ...
        # assert datetime.datetime.strptime(args.dh, '%Y-%m-%d %H:%M')
        # La date et heure de la réservation (format AAAA-MM-JJ HH:MM). Exemple: '2023-10-10 19:00' Vous devez mettre cette date entre '' ou guillemet.
        ...
        assert isinstance(int(args.pmr), int) and int(
            args.pmr) >= 0 and int(args.pmr) + int(args.bb) <= int(
            args.nbr), "Le nombre de personne(s) à mobilité doit être un nombre positif entier et inférieur au nombre de personnes"
        ...
        assert isinstance(int(args.bb), int) and int(
            args.bb) >= 0 and int(args.pmr) + int(args.bb) <= int(
            args.nbr), "Le nombre de bébé(s) par table doit être un nombre entier positif ou 0"
